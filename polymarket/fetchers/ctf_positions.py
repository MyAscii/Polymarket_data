"""
CTF position-change event fetcher.

Fetches the five Conditional Token Framework events that move position tokens:

- ERC-1155 TransferSingle / TransferBatch — every position movement, including
  off-exchange transfers, airdrops, and proxy-wallet funding that trades alone
  do not capture.
- PositionSplit / PositionsMerge / PayoutRedemption — the categorical reasons
  a position came into or out of existence (mint from collateral, burn into
  collateral, burn at settlement).

Together these are the complete on-chain record of any wallet's position state
on Polymarket.
"""
import logging
import time
...
class CtfPositionFetcher:
    """Fetch CTF position-change events with retry + bisect-on-failure.

    Same robustness pattern as the OrderFilled crawler post-fix: transient RPC
    failures are retried with exponential backoff, then the failing range is
    bisected so one bad block can't drop the whole batch.
    """
    ...
    def __init__(self, use_alchemy: bool = False, rpc_url: Optional[str] = None):
        ...
        logger.info(f"CtfPositionFetcher RPC: {url.split('/v2/')[0] if '/v2/' in url else url}")
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.warning(
                        f"CTF positions get_logs({start_block}-{end_block}) attempt "
                        f"{attempt + 1}/{max_retries} failed: {e}; retry in {sleep_for:.1f}s"
                    )
                    time.sleep(sleep_for)
        logger.error(
            f"CTF positions get_logs({start_block}-{end_block}) failed after "
    ...
    def fetch_block_range(self, start_block: int, end_block: int
        ...
        logger.info(f"Fetching CTF positions for blocks {start_block} - {end_block}")
        ...
        logger.info(f"Fetched {len(records)} CTF position events")
        return records
    ...
    def fetch_range_in_batches(self, start_block: int, end_block: int,
        ...
        while current <= end_block:
            ...
            if records is None:
                if bisect_on_failure and batch_end > current:
                    ...
                    logger.warning(
                        f"Bisecting failed CTF-positions range {current}-{batch_end} -> "
        ...
        logger.info(f"Fetched a total of {len(all_records)} CTF position events")
        return all_records
